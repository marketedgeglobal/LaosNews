...
const OUT = path.join(process.cwd(), "docs", "data", "imf_lao.json");
...
async function main() {
  ...
  for (const m of METRICS) {
    ...
    const data = await fetchJSON(`${BASE}/${m.code}/LAO?periods=${periods}`);
    const raw = data && data.values && data.values[m.code] && data.values[m.code].LAO ? data.values[m.code].LAO : {};
